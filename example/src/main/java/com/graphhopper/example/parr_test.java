...
public class parr_test {
    ...
    public static void main(String[] args) throws CsvValidationException, IOException {
        ...
        GraphHopper hopper2 = createGraphHopperInstance(relDir + "NewYork.osm.pbf");
        String outputFilePath = "parr_test.txt";
        ...
        String csvFile = "example/src/main/java/com/graphhopper/example/test.csv";
        ...
        int retult2=0;
        try {
            List<String> lines = Files.lines(Paths.get(csvFile))
                    .parallel()  // 使用并行流
                    .map(line -> processAndWrite(line, outputFilePath,hopper))
    ...
    private  static String processAndWrite(String line, String outputFilePath,GraphHopper hopper) {
        ...
        String route_list = routing(hopper, lat1, lon1, lat2, lon2);
        if (route_list !="") {
            // 模拟写入文件的操作
            String  data= route_list+"_"+nextLine[0]+","+nextLine[1]+","+nextLine[2];
            //System.out.println(data);
        /*    try {
                Files.write(Paths.get(outputFilePath), (data + System.lineSeparator()).getBytes(), java.nio.file.StandardOpenOption.APPEND);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }*/}
    ...
    public static String routing(GraphHopper hopper,double lat1,double lon1,double lat2,double lon2){//,BufferedOutputStream   outputStream)  {
